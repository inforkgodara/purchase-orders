                ...
                <div class="box box-primary">
                    <div class="box-header">
                        <div clss="col-sm-6" style="margin-bottom:2%">
                            <h3 class="box-title">
                                <span v-if="elements.total == 0 || elements.total ==1">City</span>
                            ...
                            <form @submit.prevent="search" method="POST" class="form-inline float-right search-form">
                                ...
                                <button type="submit" class="btn btn-success btn-flat"
                                    ...
                                    <router-link :to="{ name: 'create-city' }" id="add"></router-link>
                                </button>
                            </form>
                        </div>
                    </div>
                        ...
                        <table id="example2" class="table table-bordered table-hover">
                            ...
                            <tbody>
                                <tr v-for="(element) in elements.data" :key="element.id">
                                    <td>{{ element.city }}</td>
                                    <td>{{ element.province.province }}</td>
                                    <td>{{ element.country.country }}</td>
                                    <td>{{ element.updated_at }}</td>
                                    <td>{{ element.created_at }}</td>
                                    <td>
                                        <router-link :to="{ name: 'edit-city', params: {id: element.id }}">
    ...
    export default {
        ...
        data: function () {
            return {
                ...
                order: 'asc',
                limit: 25,
                pageUrl: 'admin/cities',
                searchUrl: 'admin/search-cities',
                process: false
            }
        },
        mounted() {
