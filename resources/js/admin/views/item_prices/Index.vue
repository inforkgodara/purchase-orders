                ...
                <div class="box box-primary">
                    <div class="box-header">
                        <div clss="col-sm-6" style="margin-bottom:2%">
                            <h3 class="box-title">
                                <span v-if="elements.total == 0 || elements.total ==1">Item Price</span>
                                <span v-else>Item Prices</span>
                            ...
                            <form @submit.prevent="search" method="POST" class="form-inline float-right search-form">
                                ...
                                <button type="submit" class="btn btn-success btn-flat"
                                    ...
                                    <router-link :to="{ name: 'create-item-price' }" id="add"></router-link>
                                </button>
                            </form>
                        </div>
                    </div>
                        ...
                        <table id="example2" class="table table-bordered table-hover">
                            ...
                            <tbody>
                                <tr v-for="(element) in elements.data" :key="element.id">
                                    ...
                                    <td>{{ element.effective_from }}</td>
                                    <td>{{ element.purchase_price }}</td>
                                    <td>{{ element.selling_price }}</td>
                                    <td>
                                        <router-link :to="{ name: 'edit-item-price', params: {id: element.id }}">
    ...
    export default {
        ...
        data: function () {
            return {
                ...
                columns: [{
                        'text': 'Name',
                        'real_text': 'items.name'
                    },
                    {
                        'text': 'Group',
                        'real_text': 'item_groups.group'
                    },
                    {
                        'text': 'Category',
                        'real_text': 'item_categories.category'
                    },
                    {
                        'text': 'Effective From',
                        'real_text': 'item_prices.effective_from'
                    },
                    {
                        'text': 'Purchase',
                        'real_text': 'item_prices.purchase_price'
                    },
                    {
                        'text': 'Selling',
                        'real_text': 'item_prices.selling_price'
                    }
                ],
                keyword: '',
                ...
                order: 'asc',
                limit: 25,
                pageUrl: 'admin/item-prices',
                searchUrl: 'admin/search-item-prices',
                process: false
            }
        },
        mounted() {
