                            ...
                            <div class="row">
                                <div class="col-md-3">
                                    ...
                                    <div
                                        ...
                                        <select name="location" class="form-control" v-validate="'required'"
                                            ...
                                            <option v-for="item in dropDownData.locations" :value="item.id" :key="item.id">
                                                {{ item.name }}
                                            </option>
                                        </select>
                                ...
                                <div class="col-md-3">
                                    <div
                                        ...
                                        <v-select v-model="element.creditor.name" label="name" :selectOnTab="true"
                                            ...
                                            <template #search="{attributes, events}">
                                                <input class="vs__search"
                                                    v-validate="element.creditor.name ? '' : 'required' " v-bind="attributes"
                                                    v-on="events" name="creditor"
                                                    :placeholder="element.creditor.name ? '' : 'Select creditor'" />
                                            </template>
                                        </v-select>
                                    ...
                                    <div
                                        ...
                                        <v-select v-model="element.debtor.name" label="name" :selectOnTab="true"
                                            ...
                                            <template #search="{attributes, events}">
                                                <input class="vs__search" v-bind="attributes" v-on="events"
                                                    name="debtor"
                                                    :placeholder="element.debtor.name ? '' : 'Select debtor'" />
                                            </template>
                                        </v-select>
...
<script>
    export default {
        ...
        mounted() {
            ...
            var app = this;
            this.process = true;
            console.log(this.url + app.$route.params.id + '/edit');
            axios.get(this.url + app.$route.params.id + '/edit')
                .then(function (resp) {
                    console.log(resp.data.element);
                    app.element = resp.data.element;
                    app.selectedCreditor = resp.data.creditor.name;
                    app.selectedDebtor = resp.data.debtor.name;
                    app.dropDownData = resp.data.dropdownData;
                    app.searchSelectData = {'name' : resp.data.element.name };
                    app.toShowData.category = resp.data.element.category;
                    app.toShowData.group = resp.data.element.group;
                })
                .catch(function (resp) {
                    app.error();
                })
        ...
        methods: {
            ...
            search: _.debounce((loading, search, vm) => {
                if (search) {
                    loading(true);
                    fetch(
                    `/admin/search-purchase-order?keyword=${escape(search)}`
                ).then(res => {
