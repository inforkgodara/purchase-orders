                ...
                <div class="box box-primary">
                    <div class="box-header">
                        <div clss="col-sm-6" style="margin-bottom:2%">
                            <h3 class="box-title">
                                <span v-if="elements.total == 0 || elements.total ==1">Purchase Order</span>
                                <span v-else>Purchase Orders</span>
                            ...
                            <form @submit.prevent="search" method="POST" class="form-inline float-right search-form">
                                ...
                                <button type="submit" class="btn btn-success btn-flat"
                                    ...
                                    <router-link :to="{ name: 'create-purchase-order' }" id="add"></router-link>
                                </button>
                            </form>
                        </div>
                    </div>
                        ...
                        <table id="example2" class="table table-bordered table-hover">
                            ...
                            <tbody>
                                <tr v-for="(element) in elements.data" :key="element.id">
                                    ...
                                    <td>{{ element.creditor_name }}</td>
                                    <td>{{ element.location_name }}</td>
                                    <td>{{ element.debtor_name }}</td>
                                    <td>{{ element.number }}</td>
                                    <td>{{ element.date }}</td>
                                    <td>
                                        <router-link :to="{ name: 'edit-purchase-order', params: {id: element.id }}">
        ...
        data: function () {
            return {
                ...
                columns: [{
                    ...
                    {
                        'text': 'Creditor',
                        'real_text': 'creditors.name'
                    },
                    {
                        'text': 'Location',
                        'real_text': 'locations.name'
                    },
                    {
                        'text': 'Debtor',
                        'real_text': 'debtors.name'
                    },
                    {
                        'text': 'Number',
                        'real_text': 'purchase_orders.number'
                    },
                    {
                        'text': 'Date',
                        'real_text': 'purchase_orders.date'
                    }
                ],
                keyword: '',
                ...
                order: 'asc',
                limit: 25,
                pageUrl: 'admin/purchase-orders',
                searchUrl: 'admin/search-purchase-orders',
                process: false
            }
        },
        mounted() {
